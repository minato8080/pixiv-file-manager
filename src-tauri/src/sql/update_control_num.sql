...
-- 1) 分裂が必要な (illust_id, control_num) を見つけ、
--    ディレクトリ単位（distinct save_dir）で新しい control_num を割り当てる
CREATE TEMP TABLE TEMP_CN_ASSIGN AS
WITH conflict_groups AS (
  -- 同じ illust_id & control_num なのに save_dir が異なるグループ
  SELECT i1.illust_id, i1.control_num AS old_cn
  FROM ILLUST_INFO i1
  JOIN ILLUST_INFO i2
    ON i1.illust_id = i2.illust_id
   AND i1.control_num = i2.control_num
   AND IFNULL(i1.save_dir,'') <> IFNULL(i2.save_dir,'')
  GROUP BY i1.illust_id, i1.control_num
),
group_dirs AS (
  -- 分裂対象グループの「ディレクトリ単位」の集合
  SELECT DISTINCT i.illust_id, i.control_num AS old_cn, i.save_dir
  FROM ILLUST_INFO i
  JOIN conflict_groups g
    ON g.illust_id = i.illust_id AND g.old_cn = i.control_num
),
dir_rank AS (
...
max_cn AS (
  -- illust_id ごとの既存最大 control_num
  SELECT illust_id, MAX(control_num) AS max_cn
  FROM ILLUST_INFO
  GROUP BY illust_id
),
extras AS (
...
WHERE EXISTS (
  ...
  WHERE a.illust_id = t.illust_id
    AND a.old_cn   = t.control_num
    AND IFNULL(a.save_dir,'') = IFNULL(t.save_dir,'')
    AND a.new_cn <> t.control_num
);

-- 3) ILLUST_DETAIL を必要な分だけ複製（新しい control_num にまだ無いもの）
INSERT INTO ILLUST_DETAIL (illust_id, control_num, author_id, series, character)
...
JOIN TEMP_CN_ASSIGN a
 ...
 AND a.old_cn    = d.control_num
WHERE a.new_cn <> d.control_num
  AND NOT EXISTS (
    SELECT 1 FROM ILLUST_DETAIL x
    WHERE x.illust_id = d.illust_id
      AND x.control_num = a.new_cn
  );
